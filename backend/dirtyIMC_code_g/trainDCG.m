...
%GENRE REDUCE
% TmpXSum = sum(TmpX(:,[3:4,7,9:22]),2);
% TmpYSum = sum(TmpY(:,[3:4,7,9:22]),2);
TmpX = [TmpX(:,8) TmpX(:,5) TmpX(:,1) TmpX(:,2) TmpX(:,6)];
TmpY = [TmpY(:,8) TmpY(:,5) TmpY(:,1) TmpY(:,2) TmpY(:,6)];
...
%TmpXa = [TmpXa(:,6) TmpXa(:,3) TmpXa(:,4)];
TmpYa = full(mmread("sparseYmpaa.mm.mtx"));
%TmpYa = [TmpYa(:,6) TmpYa(:,3) TmpYa(:,4)];
...
% mmwrite(filename,Y);
% filename = strcat("genre_mpaa_reduce-",DCG,"-workspace-",string(datetime('now')), '.mat');
% save(filename);
