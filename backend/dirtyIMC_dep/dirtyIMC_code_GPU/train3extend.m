...
TmpX = [TmpX TmpXa TmpXb(:,1:311)];
TmpYa = full(mmread("sparseYmpaa.mm.mtx"));
TmpY = [TmpY TmpYa TmpYb(:,1:311)];
   ...
   parfor a = 1:length(lambda)	% parameter selection
       ...
       disp(lambda(a));
        %for b = 1:length(lambda1)
           [UU SS VV U S V] = dirtyIMC(gpuArray(ObsFoldTrain), gpuArray(XFoldTrain), gpuArray(Y), lambda(a), lambda1(1));
